...
import { ImageUpload, AvatarUpload, type ImageUploadProps, AvatarUploadProps, UnstyledAvatarUpload } from '../../../../apps/admin/components/cloudinary-image';
import { classesInput, Input, InputPasswordProps, InputPinProps, InputProps, numberRegEx, numericValue } from '@repo/ui/input';
import { DatePicker, DatePickerProps } from '@repo/ui/calendar';
import { classesReactSelect, ReactSelectClassesVariant } from '@repo/ui';
...
export function useForm<TFieldValues extends FieldValues = FieldValues, TContext = any>(props: UseFormProps<TFieldValues, TContext> = {}) {
  const { resolver, schema, disabled, ...rest } = props;
  ...
  const form = useFormPrimitive({ resolver: resolver || (schema && zodResolver(schema)), disabled: disabled || loading, ...rest });
...
export const FormItem = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & { unstyled?: boolean }>(({ className, unstyled, ...props }, ref) => {
  ...
  return (
    <FormItemContext.Provider value={{ id }}>
      <div ref={ref} className={twMerge(!unstyled && 'relative flex flex-col gap-2', className)} {...props} />
...
export const FormInputField = React.forwardRef<HTMLInputElement, FormInputFieldProps>((_props, ref) => {
  ...
  const messageInPlaceholder = error && typeof errorMessage === 'string' && errorMessage === 'placeholder';
  const placeholderMessage = messageInPlaceholder ? (error.message ?? placeholder) : placeholder;
  const isValidField = isValidValue(_value, invalidValues);
...
export const formCardVariant = cvx({
  assign: 'relative rounded-2xl border dark:border-0 bg-card text-color shadow-sm px-6 py-4 grid grid-cols-1 gap-4',
  variants: {
    grid: {
      lg2: 'lg:grid-cols-2'
    }
