...
export function renderFormField<TControl extends Rhf.FieldValues, TName extends string, TField extends FormFields<TName>>(
  ...
  return (
    <FormField
      ...
      render={args => {
        const rest = omit(field, ['formField', 'onChange']);
        const props = {
          ...rest,
          disabled: loading,
          ...args.field,
          onChange: (event: any) => {
            args.field.onChange(event);
            (field as any)?.onChange?.(event);
          }
        ...
        switch (field.formField) {
          ...
          case 'date':
            return <FormDateField {...(props as FormFieldMap<TName>['date'])} />;
