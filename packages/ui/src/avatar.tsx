...
const classes = cvx({
  variants: {
    selector: {
      root: 'relative flex items-center justify-center select-none overflow-hidden p-0',
      image: 'object-cover size-full block absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2',
      fallback: 'absolute flex size-full items-center justify-center font-medium select-none text-[calc(var(--avatar-fz))]',
...
function getStyles(selector: SelectorGroup, opts?: Options) {
  ...
  return {
    ...
    style: ocx(
      ...
      selector === 'root' && [
        { '--avatar-fz': `calc(var(--avatar-size) / max(${opts?.lengthInitial}, 2.75))` },
        opts?.round && { '--avatar-round': rem(opts?.round) },
        opts?.color && [{ '--avatar-bg': opts.color }, isInitials && { '--avatar-text-color': 'var(--text-color)' }]
      ],
