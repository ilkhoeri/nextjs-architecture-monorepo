...
const classes = cvx({
  variants: {
    selector: {
      ...
      scrollbar: 'flex touch-none select-none p-0.5 bg-[var(--bg)] ease-out transition-colors [transition-duration:160ms]',
      thumb:
        'relative flex-1 rounded-full bg-[var(--sa-thumb-color)] cursor-grab active:cursor-grabbing before:absolute before:size-full before:left-1/2 before:top-1/2 before:-translate-x-1/2 before:-translate-y-1/2 before:min-h-11 before:min-w-[250%]',
      corner: 'bg-[var(--sa-track-color,transparent)]'
    },
    viewport: {
      vertical: 'flex-col',
      horizontal: 'flex-row'
    ...
    scrollbar: {
      vertical: 'w-[var(--sa-thumb-size)]',
      horizontal: 'flex-col h-[var(--sa-thumb-size)]'
    }
  }
});
...
function getStyles(selector: __Selector, options: Options = {}) {
  ...
  return {
    ...
    className: cn(
      ...
      scrollbar && !isUnstyled && ['[&:not(:empty)]:[--bg:var(--sa-track-color,transparent)]'],
      classNames?.[selector],
      className
    ),
    style: ocx(
