  ...
  return (
    ...
    >
      <Svg currentFill="fill" size="calc(var(--burger-size)/1.333)" {...getStyles({ classes: "svg", open: openChange })}>
        {[...Array(3)].map((_, index) => (
          <path key={index} {...getStyles({ path: String(index) as any, open: openChange, index })} />
        ))}
      </Svg>
    </UnstyledButton>
  );
...
function getStyles(selector: Selector & Options = {}) {
  ...
  return {
    ...
    d: (path ? burger({ path: String(index) as any }) : undefined) as React.SVGAttributes<SVGPathElement>["d"],
    style: {
      ...(path
        ? {
            transition: "transform .35s ease",
            transform: open ? burger({ isOpen: String(index) as any }) : "none"
          }
        : undefined),
...
const burger = cvx({
  variants: {
    classes: {
      root: "size-[var(--burger-size)] rounded-[calc(var(--burger-size)/5.333)] text-[var(--burger-color)] border-solid border-[var(--burger-color)] flex items-center justify-center relative cursor-pointer outline-none focus-visible:outline-0",
