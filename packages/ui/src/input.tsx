...
export const classesInput = cvx({
  assign:
    'inline-flex items-center whitespace-nowrap transition-colors disabled:pointer-events-none w-full pl-3 text-left font-normal placeholder:text-muted-foreground disabled:cursor-not-allowed disabled:opacity-50',
  variants: {
    variant: {
      outline: 'hover:bg-[#ffffff0f] focus-visible:bg-[#ffffff0f] border border-border focus-visible:border-ceramic-dimmed focus-visible:ring-0 focus-visible:outline-none',
      'outline-ring':
    ...
    size: {
      sm: 'h-[30px] rounded-[0.375rem] py-[0.375rem] px-3 text-tiny file:text-tiny file:font-medium',
      lg: 'h-[51px] rounded-[9px] py-2 px-5 text-[13px] font-medium file:text-[13px] placeholder:text-[13px]'
    }
  }
});
const classesButtonPassword = cvx({
  assign: 'absolute right-0 bottom-0 !bg-transparent flex items-center justify-center cursor-pointer z-99 border-0 outline-0 pointer-events-auto [&>svg]:transition-colors',
  variants: {
    variant: {
      outline: 'text-color',
    ...
    size: {
      sm: 'size-[30px] rounded-[0.375rem] [--size-icon:16px]',
...
export const InputPin = React.forwardRef<HTMLInputElement, InputPinProps>((_props, ref) => {
  ...
  const [inputPin, setInputPin] = React.useState<InputPinValue>(value ?? __defaultValue);
  ...
  const handleChange = (index: number, inputValue: string) => {
    ...
    let newPin: InputPinValue = '';

    if (Array.isArray(inputPin)) {
      newPin = [...inputPin];
      const numeric = inputValue.replace(/[^0-9]/g, '');
      const alphanumeric = inputValue.replace(/[<'|">[\]{}?/,.`\\%^&~:;*()+$#@!_\-\+=]/g, '');
      newPin[index] = isNumeric ? numeric : alphanumeric;
    }
    if (typeof inputPin === 'string') {
      newPin = inputPin;
    }
    ...
    if (onChange) {
      onChange(fakeEvent(Array.isArray(newPin) ? newPin.join('') : newPin));
    }
  };
