...
export function SheetsProvider(_props: SheetsProviderProps) {
  ...
  const [_open, _setOpen] = React.useState(defaultOpenBoolean);
...
const styleDefault = {
  ...
  content: cvx({
    assign:
      'fixed z-[111] gap-4 bg-background p-6 shadow-lg transition ease-linear data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=open]:duration-200 data-[state=closed]:duration-200 border-border',
    variants: {
      side: {
...
function styleByVariant(side: `${SheetsSide}`) {
  return cvx({
    variants: {
      ...
      item: {
        accordion:
          'group relative flex flex-col h-auto border-0 select-none gap-[var(--offset)] data-[side=top]:flex-col-reverse data-[side=right]:flex-row data-[side=bottom]:flex-col data-[side=left]:flex-row-reverse data-[align=start]:items-start data-[align=center]:items-center data-[align=end]:items-end border-b border-border',
        collapsible: '',
        dialog: '',
        drawer: '',
        dropdown: ''
      ...
      trigger: {
        ...
        dropdown: cn(styleDefault.trigger)
      },
      content: {
