...
function generateTableOfContents(basePath: string, title: string, files: string[] = tocList) {
  let toc = `# ${title}\n\n---\n\n`;

  files.forEach(file => {
    const filePath = path.join(basePath, `${file}.mdx`);
...
function generateTocFile(basePath: string, { title, date }: GenerateTocFileProps) {
  try {
