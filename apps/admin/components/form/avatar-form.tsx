...
import { SettingAvatarImageSchema } from '@/schemas/user';
import { CameraRotateIcon } from '@repo/icons';
import { Form, useForm } from '@repo/components/fields/form';
import { MotionImage } from '@repo/components/motion';
...
export function SettingAvatarForm({ account }: { account: User }) {
  ...
  async function onSubmit(value: SettingsFormValues) {
    setLoading(true);
    try {
      await axios.patch(`/api/auth/update/${account?.id}`, value);
      router.refresh();
    } catch (error: any) {
      toast.error('Error');
    } finally {
