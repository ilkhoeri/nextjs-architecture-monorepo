...
import { FORM_USER, User } from '@/types/user';
import { isValidURL } from '@repo/utils/text-parser';
import { Button, buttonVariants } from '@repo/ui/button';
import { handlerLinkUser } from '@/auth/handler/user-link';
import { useNameFromURL } from '@repo/utils/icons-preset';
import { Form, FormCard, useForm } from '@repo/components/fields/form';
import { containerVariants, itemVariants } from '@repo/components/motion';
import { AddCircleFillIcon, ArrowUpDownRightFillIcon, PencilFillIcon, TrashFillIcon } from '@repo/icons';

interface AccountLinksProps {
  account: User;
  links: NonNullable<User>['links'];
  className?: string;
}
...
type LinkFormProps = FORM_USER.FORM_LINK & { setOpenId?: (value: string | null) => void; account?: User };
  ...
  return isFieldData ? (
    ...
    <>
      ...
      <motion.button
        ...
        disabled={loading}
        className={cn(buttonVariants(), styleForm().auth().submit(), 'gap-3 w-full overflow-hidden text-white')}
      >
        <AddCircleFillIcon size={!addLink ? 22 : '0px'} color="white" /> Add New
      </motion.button>
    </>
  );
}
